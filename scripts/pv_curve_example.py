...
def main(cfg: DictConfig):
    ...
    for idx, case in conditions.iterrows():
        label = f"$G_{{eff}}$ {case['Geff']} $W/m^2$\n$T_{{cell}}$ {case['Tcell']} $\\degree C$"
    ...
    def draw_arrow(ax, label, x0, y0, rotation, size, direction):
        style = f"{direction}arrow"
        bbox_props = dict(boxstyle=style, fc=(0.8, 0.9, 0.9), ec="b", lw=1)
        t = ax.text(x0, y0, label, ha="left", va="bottom", rotation=rotation, size=size, bbox=bbox_props, zorder=-1)
